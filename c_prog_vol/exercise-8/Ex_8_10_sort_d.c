...
static int fold_g;  /* check for fold option */
static int dir_g;    /* check directory option */
...
int main(int argc, char **argv)
{
    ...
    if (argc == 3) {
        if(strcmp(argv[1],"-d")) /* check for directory option */
            dir_g = 0;
        else
            dir_g = 1;
        ...
        if(strcmp(argv[2], "-f"))  /* check for fold option */
            fold_g = 0;
        else 
            fold_g = 1;
    }
...
int stringcmp(char *s1,char *s2)
{
    ...
    /* changes here */
    if ( dir_g == 1) {    /* ignore special characters */
